...
def create_term_loan(session: Session, **kwargs) -> LoanData:
    loan = Loan(
...
def get_term_loan(session: Session, user_id: int, card_type: str) -> LoanData:
    term_loan = (
        session.query(LoanData)
        .join(Product.product_name == card_type)
        .filter(
